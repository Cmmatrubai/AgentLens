...
describe("projectExecutionGraph", () => {
  ...
  it("reuses the exact compatible lifecycle pair key and preserves member identity", () => {
    ...
    expect(compact.nodes).toEqual([{
      ...
      estimatedHeight: 160,
      recorderTiming: {
        state: "available", elapsedMs: 1000, basis: "recorder_received_at",
        provenance: "derived", supportingEventIds: ["command-start", "command-terminal"]
      }
    }]);
  ...
  it("splits lifecycle pairs that contain failures or loaded relationship endpoints", () => {
    ...
    for (const [startIndex, terminalIndex] of [[0, 1], [2, 3], [5, 6]]) {
      expect(graph.nodes[startIndex!]?.recorderTiming).toBeUndefined();
      expect(graph.nodes[terminalIndex!]?.recorderTiming).toEqual({
        state: "available", elapsedMs: 1000, basis: "recorder_received_at",
        provenance: "derived",
        supportingEventIds: [events[startIndex!]!.eventId, events[terminalIndex!]!.eventId]
      });
    }
