...
export function ExecutionNode(props: Readonly<{
  ...
  const timing = node.recorderTiming?.state === "available"
    ? `Recorder-observed elapsed · derived from receipt timestamps · ${node.recorderTiming.elapsedMs.toLocaleString()} ms`
    : null;
  return <div ref={props.nodeRef} role="option" tabIndex={props.tabIndex}
    aria-selected={selectedMember !== undefined}
    aria-label={`${active.safeSummary}. ${provenanceLabels[active.provenance]}. ${eventStatus(active)}. ${timing ? `${timing}. ` : ""}Use Left and Right Arrow for recorded branches. ${actions.length > 1 ? `Use Alt+Left and Alt+Right to choose a group action. Actions: ${actions.map((action) => action.label).join("; ")}. ` : ""}Current action: ${actions[index]!.label}.`}
    ...
    <article className="execution-node__card">
      ...
      {node.recorderTiming?.state === "available" && <div className="trajectory-row__times">
        <span>Recorder-observed elapsed · derived from receipt timestamps</span>
        <span>{node.recorderTiming.elapsedMs.toLocaleString()} ms</span>
      </div>}
