...
export function NodeClarification(props: Readonly<{
  ...
  return <motion.section className="node-clarification" aria-label="Node clarification"
    key={event.eventId} initial={{ opacity: 0, y: policy.reduced ? 0 : 4 }} animate={{ opacity: 1, y: 0 }}
    ...
    {node.type !== "event" && <div className="node-clarification__actions" aria-label="Immutable group members">
      ...
      <button type="button" data-row-action="expand" onClick={() => props.onExpand(node.key)}>{node.expanded ? "Collapse" : "Expand"} {node.type === "lifecycle_group" ? "lifecycle" : "routine"} events</button>
    </div>}
