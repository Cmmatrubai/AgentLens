...
Also assert a second recovery call is idempotent by relationship and does not append a duplicate. An observed canonical `completed`/`failed`/`declined`/`interrupted` status, or an exact native `item`/`tool` terminal event type with `unknown` status, blocks both automatic and manual recovery; a same-identity `item.progress`/`unknown` observation remains open. Assert the public storage surface exposes neither the raw SQLite connection nor a repository database handle that could bypass append-only event APIs.
...
Keep child argv bytes/ordering exact. Buffer every non-TTY stdin stream fully in memory, capture it under policy, and forward the original bytes unchanged. Use Node `spawn` with argument arrays for the child and `execFile` with argument arrays for exact read-only Git commands. For every AgentLens-owned Git subprocess, set `GIT_OPTIONAL_LOCKS=0` and `GIT_NO_LAZY_FETCH=1`, force every supported Git trace destination to `0`, and override `core.fsmonitor=false`; add `--no-ext-diff --no-textconv` to tracked diff capture. Query effective config before both status/diff phases and refuse a configured `filter.*.clean` or `filter.*.process` command. Capture porcelain status as a NUL-delimited byte stream; use exact bytes for path containment and metadata, but persist only valid UTF-8 display or a content-free placeholder. Refuse dirty/non-Git before child spawn. Canonicalize the repository and prospective data-root path (including the nearest existing ancestor) and reject equality/containment before storage creation or spawn.
